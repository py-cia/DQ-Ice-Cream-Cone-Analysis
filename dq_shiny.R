  ...
  output$poly <- renderPlot({
    if (input$name == "All") {
      ggplot(data = df_all(), mapping = aes(x = Weight)) +
        ...
        labs(x = paste("Weight", ounce_print()), y = "Count", title = paste(input$size, "Cone Distribution")) +
    ...
    ggplot(data = df(), aes(x = Weight)) +
      geom_histogram(binwidth = .1, fill = "blue") +
      labs(x = paste("Weight", ounce_print()), y = "Count", title = paste("Cone Distribution for", input$name)) +
      theme(plot.caption = element_text(size = 19),
      axis.title = element_text(face = "bold"),
      plot.title = element_text(face = "bold"),
      panel.background = element_rect(fill = "white"),
